...
// Only known filter keys are carried back; never accept a redirect URL.
const filters=new URLSearchParams(query.get('filters')||'');
const backQuery=new URLSearchParams();
for(const key of ['brand','model','fuel','maxPrice','sort']) {
  const value=filters.get(key);if(value && value.length<=100)backQuery.set(key,value);
}
const catalogUrl=`vehiculos.html${backQuery.size?'?'+backQuery:''}`;
...
if(!vehicle) {
  ...
  function show(next){
    ...
    const strip=document.querySelector('.gallery-thumbs'),active=thumbs[index];
  ...
  document.querySelector('.gallery-open').addEventListener('click',()=>{if(Date.now()>suppressOpenUntil)dialog.showModal();});
  dialog.querySelector('.dialog-close').addEventListener('click',()=>dialog.close());
  const galleryKeys=e=>{if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();show(index+(e.key==='ArrowRight'?1:-1));}};
  dialog.addEventListener('keydown',galleryKeys);
  document.querySelector('.detail-gallery').addEventListener('keydown',galleryKeys);
  ...
  for(const el of [document.querySelector('.gallery-open'),large]) {
    ...
    el.addEventListener('touchend',e=>{if(!start)return;const t=e.changedTouches[0],dx=t.clientX-start.x,dy=t.clientY-start.y;start=null;if(Math.abs(dx)>55&&Math.abs(dx)>Math.abs(dy)*1.5){suppressOpenUntil=Date.now()+500;show(index+(dx<0?1:-1));}},{passive:true});
