...
function closeEditor(){if(busy)return;if(dirty&&!confirm('¿Descartar los cambios sin guardar de esta ficha?'))return;editor.close();dirty=false;releaseEditorFiles();}
...
$('#confirmDelete').addEventListener('click',async()=>{const button=$('#confirmDelete');button.disabled=true;try{const old=fleet.find(v=>v.id===pendingDelete);const saved=await saveVehicle({...old,status:'archived'},old);fleet=fleet.map(v=>v.id===saved.id?{...v,status:saved.status,version:saved.version}:v);pendingDelete=null;deletion.close();render();$('#panelFeedback').textContent='Vehículo retirado. Puedes recuperarlo cambiando su estado.';}catch(error){deletion.close();$('#panelFeedback').textContent=error.message;}finally{button.disabled=false;}});
...
$('#photoUpload').addEventListener('change',async e=>{
 const run=++uploadRun;processingPhotos=true;const submit=form.querySelector('[type=submit]');submit.disabled=true;
 const errors=[];
...
form.addEventListener('submit',async e=>{
 ...
 try{
  for(const photo of v.gallery){if(pendingUploads.has(photo.src)){photo.path=await uploadPhoto(pendingUploads.get(photo.src));uploadedInEditor.add(photo.path);const original=photos.find(p=>p.src===photo.src);if(original)original.path=photo.path;pendingUploads.delete(photo.src);}}
  const saved=await saveVehicle(v,old);
  // Apply the confirmed write locally before any follow-up request can fail.
  const updated=(await resolvePhotos([saved]))[0];
  fleet=old?fleet.map(item=>item.id===saved.id?updated:item):[updated,...fleet];
